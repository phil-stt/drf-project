...
class StreamPlatformTestCase(APITestCase):
    ...
    def test_streamplatform_create(self):
        ...
        # Regular user cannot create a stream platform, thus the 403 expected response
    ...
    def test_streamplatform_update(self):
        ...
        # Regular user cannot update a stream platform, thus the 403 expected response
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_streamplatform_delete(self):
        url = reverse('streamplatform-detail', kwargs={'pk': self.stream.id})
        response = self.client.delete(url)
        # modified below since it doesn't include trailing slash which causes a 200 response 
        # response = self.client.delete(reverse('streamplatform-detail', args=(self.stream.id,)))
        # Standard user should not be able to delete a stream platform
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN) 
...
class WatchListTestCase(APITestCase):
    ...
    def test_watchlist_delete(self):
        url = reverse('movie-detail', kwargs={'pk': self.watchlist.id})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN) 
...
class ReviewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.watchlist2 = models.WatchList.objects.create(platform=self.stream, title="Example Movie 2",
                                storyline="Example Storyline 2", active=True)
    ...
    def test_review_create(self):
        ...
        # Should create an error because a user cannot review the same movie twice
        response = self.client.post(reverse('review-create', args=(self.watchlist.id,)), data)
    ...
    def test_review_update(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(models.Review.objects.get().description, 'Great Movie! - Updated')
